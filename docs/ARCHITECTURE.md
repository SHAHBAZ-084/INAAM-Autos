# INAAM AUTOS & SPARE PARTS — Architecture

Offline Windows desktop POS for an auto parts shop (**INAAM AUTOS & SPARE PARTS**). The same codebase is reusable for other businesses via developer settings (brand, print fields, starter categories).
...
| Database | SQLite via Prisma 6 (`backend/prisma/data/inaam-autos.db`) |
...
2. **Products and Inventory** — auto parts / products, simple variants, stock  
