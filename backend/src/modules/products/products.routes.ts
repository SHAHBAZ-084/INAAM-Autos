...
productsRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const category = await productsService.updateProductCategory(id, req.body.name);
    res.json(category);
  }),
);
...
productsRouter.get(
  ...
  asyncHandler(async (_req, res) => {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="inaam-autos-products-template.xlsx"');
    res.send(buffer);
  }),
);
