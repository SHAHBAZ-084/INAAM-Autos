...
/** Starter product categories for a new shop. Owners can add, rename, or delete these. */
export const STARTER_PRODUCT_CATEGORIES: { name: string; code: string }[] = [
  { name: 'Engine Parts', code: 'ENG' },
  { name: 'Body Parts', code: 'BDY' },
  { name: 'Electrical', code: 'ELC' },
  { name: 'Batteries', code: 'BAT' },
  { name: 'Tyres & Wheels', code: 'TYR' },
  { name: 'Lubricants & Oils', code: 'OIL' },
  { name: 'Accessories', code: 'ACC' },
];

/** @deprecated Use STARTER_PRODUCT_CATEGORIES — kept so existing imports keep compiling. */
export const DEFAULT_PRODUCT_CATEGORIES = STARTER_PRODUCT_CATEGORIES;

/** Idempotent seed of starter product categories (only when the shop has none yet). */
export async function ensureDefaultProductCategories() {
  const existingCount = await prisma.productCategory.count();
  if (existingCount > 0) return;

  for (const row of STARTER_PRODUCT_CATEGORIES) {
...
export async function updateProductCategory(id: number, name: string) {
  ...
  if (!existing) throw new AppError(404, 'Product category not found');
  try {
...
export async function deleteProductCategory(id: number) {
  ...
  if (inUse > 0) {
    throw new AppError(400, 'Cannot delete a category that still has products. Reassign them first.');
  }
