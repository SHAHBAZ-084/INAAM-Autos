...
if (!envExisted || !envLooksValid(envPath)) {
  ...
  // Keep an existing .env, but retarget the SQLite file if it still uses the old garments name.
  try {
    const envText = fs.readFileSync(envPath, 'utf8');
    if (envText.includes('usman-garments.db')) {
      fs.writeFileSync(envPath, envText.replace(/usman-garments\.db/g, 'inaam-autos.db'), 'utf8');
      console.log('Updated DATABASE_URL in backend/.env to inaam-autos.db');
    }
  } catch {
    /* leave .env as-is */
  }
  console.log('backend/.env already exists — leaving other values unchanged');
}
