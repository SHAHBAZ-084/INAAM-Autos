 ...
 * %AppData%\INAAM AUTOS and is only touched by runSafeMigrations on next launch.
 */
...
const drillRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'inaam-manual-upgrade-'));
const userData = path.join(drillRoot, 'userData');
const dbPath = path.join(userData, 'inaam-autos.db');
...
async function main() {
  ...
  process.env.INAAM_USER_DATA = userData;
  ...
  assert(fs.existsSync(path.join(good.backup.folderPath, 'inaam-autos.db')), 'Backup DB missing');
