 ...
 *   - Invoice barcode (alphanumeric, matches invoicePrefix e.g. IA-000001) -> /sales/return
 */
export function useGlobalBarcodeScanner() {
  ...
  useEffect(() => {
    ...
    const off = onSettingsUpdated(handleSettingsUpdated);
    return off;
  }, []);
  ...
  useEffect(() => {
    ...
    const processGlobalScan = (scannedCode: string) => {
      ...
      // Matches configured prefix (e.g. IA-000001 -> IA000001) OR general prefix pattern (e.g. PREFIX-123456)
