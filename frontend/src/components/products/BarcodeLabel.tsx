...
const LABEL_LAYOUTS: Array<{ key: LabelLayoutKey; label: string; hint: string }> = [
  { key: 'standard', label: 'Standard', hint: 'Shop · name · variant · price · barcode' },
  ...
  return (
    ...
    >
      ...
      {(item.customLines ?? []).map((line) => (
        <p key={line} className={`font-semibold text-black ${compact ? 'text-[8px]' : 'text-[10px]'}`}>
          {line}
        </p>
      ))}
...
export function buildStickerLabelPrintHtml(
  ...
  const packedRows = packLabelsAcross(items, labelsAcross);

  const labelMarkup = (item: LabelItem) =>
    `<div class="label">${labelInnerHtml(item, layout, compact, renderOpts)}</div>`;
  const emptyLabelMarkup = `<div class="label empty" aria-hidden="true"></div>`;

  // Single-lane rolls: preserve exact legacy output (one .label = one @page).
  if (labelsAcross <= 1) {
    const pages = items.map((item) => labelMarkup(item)).join('');
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  const pageWidthMm = stickerPageWidthMm(widthMm, labelsAcross, acrossGapMm);
  const pages = packedRows
    .map((row) => {
      const lanes = row.map((item) => (item ? labelMarkup(item) : emptyLabelMarkup)).join('');
      return `<div class="page">${lanes}</div>`;
    })
    .join('');

  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"/><title>Barcode Labels ${widthMm}x${heightMm} x${labelsAcross}</title>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  html, body {
...
/**
 * Freeform custom style print path — absolute-positioned fields from CustomLabelStyle.
 * Separate from the flex-based buildStickerLabelPrintHtml layouts.
 * Delegates document HTML to labelStyleRender so designer preview matches print byte-for-byte.
 */
export function buildFreeformStickerHtml(
  items: LabelItem[],
  style: Pick<CustomLabelStyle, 'canvasWidthMm' | 'canvasHeightMm' | 'fields'>,
  labelsAcross = 1,
  acrossGapMm = 0,
): string {
  return buildFreeformPrintDocumentHtml(
    items.map((item) => ({
...
function buildPrintHtml(
  ...
  if (size.mode === 'thermal') {
    return buildStickerLabelPrintHtml(
      items,
      layout,
      size.widthMm,
      size.heightMm,
...
<style>
  * { box-sizing: border-box; }
  ...
  .custom { font-size: 7pt; font-weight: 700; color: #222; margin: 0.5mm 0 0; line-height: 1.1; }
...
export type PrintLabelsOptions = {
  ...
  /** When set, thermal prints use freeform absolute layout instead of built-in flex layouts. */
  customStyle?: Pick<CustomLabelStyle, 'canvasWidthMm' | 'canvasHeightMm' | 'fields'> | null;
...
export async function printBarcodeLabels(
  ...
  const stickerWidthMm = customStyle && isSticker ? customStyle.canvasWidthMm : size.widthMm;
  const stickerHeightMm = customStyle && isSticker ? customStyle.canvasHeightMm : size.heightMm;
  const pageWidthMm = stickerPageWidthMm(stickerWidthMm, labelsAcross, acrossGapMm);
  const pageSize = isSticker
    ? stickerWidthMm === 58 && stickerHeightMm === 40 && labelsAcross <= 1 && !customStyle
      ? LABEL_58X40_MICRONS
      : { width: pageWidthMm * 1000, height: stickerHeightMm * 1000 }
    : 'A4';
...
export function BarcodeLabelModal({
  ...
  title?: string;
  labelSizeKey?: string;
  /** Default style selection: `builtin:standard` or `custom:<key>` (mirrors barcodeLabelSize). */
  ...
  const [customStyles, setCustomStyles] = useState<CustomLabelStyle[]>([]);
  /** `builtin:<layout>` or `custom:<styleKey>` */
  const [styleSelection, setStyleSelection] = useState(
    () => resolveLabelStyleSelection(labelStyleKey),
  );
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([api.listCustomLabelPresets(), api.listCustomLabelStyles()])
      .then(([presets, styles]) => {
        if (cancelled) return;
  ...
  const selectedCustomStyle = useMemo(() => {
    if (!styleSelection.startsWith('custom:')) return null;
    const key = styleSelection.slice('custom:'.length);
    return customStyles.find((s) => s.key === key) ?? null;
  }, [styleSelection, customStyles]);

  const labelLayout: LabelLayoutKey = styleSelection.startsWith('builtin:')
    ? ((styleSelection.slice('builtin:'.length) as LabelLayoutKey) || 'standard')
    : 'standard';
  ...
  const packingState = useMemo(() => {
    try {
      ...
      return { queue, packedRows, stats, error: '' };
    } catch (err) {
      return {
        queue: [] as LabelItem[],
        packedRows: [] as PackedLabelRow<LabelItem>[],
        stats: summarizePackedLabels([], size.labelsAcross ?? 1),
  ...
  useEffect(() => {
    if (labelStyleKey?.trim()) setStyleSelection(labelStyleKey.trim());
  }, [labelStyleKey]);

  // Once custom styles load (or change), drop a deleted custom default.
  useEffect(() => {
    setStyleSelection((prev) =>
      resolveLabelStyleSelection(
        prev,
        customStyles.map((s) => s.key),
      ),
    );
  }, [customStyles]);
  ...
  async function runPrint(list: LabelItem[], preview = false) {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        const across = size.labelsAcross ?? 1;
        const rows = packLabelsAcross(list, across).length;
        setPrintMessage(
          `Sent ${list.length} label${list.length === 1 ? '' : 's'} in ${rows} row${rows === 1 ? '' : 's'} to ${result.printer || 'printer'}.`,
        );
      }
    } catch (err) {
      ...
      >
        <div className="shrink-0 border-b border-border px-4 py-3">
          ...
          <p className="mt-1 text-sm text-textSecondary">
            ...
            {size.mode === 'thermal'
              ? ` · ${packingStats.across} across smart packed`
              : ''}
          </p>
          <p className="mt-1 text-xs text-textMuted">
            Across: {packingStats.across} · Total Labels: {packingStats.totalLabels} · Rows:{' '}
            {packingStats.rows} · Physical Slots: {packingStats.physicalSlots} · Unused Slots:{' '}
            {packingStats.unusedSlots}
          </p>
          ...
          {packingState.error ? (
            <p className="mt-2 text-xs font-medium text-danger">{packingState.error}</p>
          ) : null}
          {printMessage ? (
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-3">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <select
                ...
                onChange={(event) => setStyleSelection(event.target.value)}
              >
                <optgroup label="Built-in layouts">
                  {LABEL_LAYOUTS.map((style) => (
                    <option key={style.key} value={`builtin:${style.key}`}>
                      {style.label}
                    </option>
                  ))}
                </optgroup>
              ...
              <p className="mt-1 text-xs text-textMuted">
                {selectedCustomStyle
                  ? `Custom freeform · ${selectedCustomStyle.canvasWidthMm}×${selectedCustomStyle.canvasHeightMm} mm`
                  : layoutMeta.hint}
              </p>
            </div>
          </div>
          ...
          <div className="mt-3 space-y-3">
            {previewRows.map((row, rowIndex) => (
              <div
                ...
                style={{ gridTemplateColumns: `repeat(${packingStats.across}, minmax(0, 1fr))` }}
              >
                {row.map((item, slotIndex) =>
                  item ? (
                    selectedCustomStyle ? (
                      <div
                        key={item.key}
                        className="overflow-hidden rounded border border-border bg-white p-1"
                        dangerouslySetInnerHTML={{
                          __html: buildFreeformCanvasInnerHtml(
                            {
                              businessName: item.businessName,
                              productName: item.productName,
                              size: item.size,
                    ...
                    <div
                      ...
                      className="flex min-h-24 items-center justify-center rounded border border-dashed border-border bg-surface1 text-xs text-textMuted"
                    >
                      Empty
                    </div>
                  ),
          ...
          {packedRows.length > previewRows.length ? (
            <p className="mt-2 text-xs text-textSecondary">
              Preview shows the first {previewRows.length} packed row{previewRows.length === 1 ? '' : 's'}. Print uses the same packed layout data.
            </p>
          ) : null}
        </div>
        ...
        <div className="shrink-0 border-t border-border bg-surface2 px-4 py-3">
          <div className="flex flex-wrap justify-end gap-2">
            ...
            <SecondaryButton
              type="button"
              disabled={printing || printable.length === 0 || Boolean(packingState.error)}
