...
export type BusinessSettings = {
  ...
  /** Default print style key: `builtin:standard` etc. or `custom:<styleKey>`. */
  ...
  developerConfig?: import('../config/developerPrint').DeveloperPrintConfig;
  createdAt: string;
  updatedAt: string;
};
...
export type LabelStyleField = {
  ...
  /** Degrees 0–359; visual transform only (box size unchanged). */
  rotationDeg?: number;
  /** Print-safe system font; defaults to Arial. */
  fontFamily?: string;
  fontStyle?: 'normal' | 'italic';
};

export type CustomLabelStyle = {
  id: number;
  key: string;
  name: string;
...
export const api = {
  ...
  listCustomLabelStyles() {
    return request<CustomLabelStyle[]>('/api/settings/custom-label-styles');
  },
  createCustomLabelStyle(input: {
    name: string;
    canvasWidthMm: number;
    canvasHeightMm: number;
    fields: LabelStyleField[];
  ...
  updateCustomLabelStyle(
    ...
    return request<CustomLabelStyle>(`/api/settings/custom-label-styles/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(input),
    });
  },
  deleteCustomLabelStyle(id: number) {
    return request<CustomLabelStyle>(`/api/settings/custom-label-styles/${id}`, {
      method: 'DELETE',
    });
  },
