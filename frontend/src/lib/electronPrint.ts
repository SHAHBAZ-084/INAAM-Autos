import { getDesktopApi } from '../config/brand';
...
export function isElectronPrintAvailable(): boolean {
  return typeof window !== 'undefined' && typeof getDesktopApi()?.printHtml === 'function';
}
...
export async function printHtmlDocument(
  ...
  try {
    const result = await getDesktopApi()!.printHtml!({
      html,
    ...
    void getDesktopApi()?.restoreWindowInput?.();
  }
}

export async function listPrinters() {
  if (!isElectronPrintAvailable() || !getDesktopApi()?.listPrinters) return [];
  try {
    return await getDesktopApi()!.listPrinters!();
  } catch {
    return [];
  }
}
